...
impl LazyRoute for ClassSettingRoute {
    fn data() -> Self {
        // Blocking: academic_setting() also performs the page's auth check
        // and calls redirect() on failure, which must land before the
        // response headers are sent — a non-blocking resource doesn't
        // guarantee that.
