...
pub fn ViewQuestions() -> impl IntoView {
    ...
    let submit_question = move |ev: leptos::ev::SubmitEvent| {
        ...
        let chapter_id = chapter_id_u32.get_untracked();
        let class_id = class_id_u32.get_untracked();
        let subject_id = subject_id_u32.get_untracked();
    ...
    view! {
        <div class="p-2 sm:p-0">
            ...
            >
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-lg font-semibold">
                        {move || {
                            let name = selected_chapter_name.get();
                            if name.is_empty() {
                                "Questions".to_string()
                            } else {
                                format!("Questions in: {}", name)
                            }
                        }}
                    </h2>
                    <Button
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <For
                            ...
                            children=move |question| {
                                view! { <QuestionCard question=question /> }
                            }
                        />
                    </div>
                </Show>
            ...
            // --- Add Question Dialog ---
            <Dialog open=question_dialog_open>
                <DialogTrigger slot>
                    <div></div>
                </DialogTrigger>
                <DialogClose />
...
#[component]
fn QuestionCard(question: Question) -> impl IntoView {
    let expanded = RwSignal::new(false);
    let question = StoredValue::new(question);
    let variant = match question.get_value().question_type {
        crate::domain::question::QuestionType::Objective => "default",
        crate::domain::question::QuestionType::Subjective => "secondary",
    };
    ...
    view! {
        <Card class="cursor-pointer hover:shadow-md transition-shadow">
            <div class="p-4" on:click=move |_| expanded.set(!expanded.get())>
                ...
                <p class="text-sm font-medium line-clamp-3">
                    {question_text}
                </p>
